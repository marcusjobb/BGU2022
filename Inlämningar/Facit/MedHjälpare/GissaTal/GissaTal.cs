    ...
    public void Start()
    {
        ...
        while(KeepPlaying)
        {
            ...
            while(guesses < 10)
            {
                ...
                if((helperHigh - helperLow) > 1)
                    Console.WriteLine($"Försök med ett tal mellan {helperLow} och {helperHigh}");
                else
                    Console.WriteLine($"FFS! Försök med {helperLow + 1}");
                ShowStats(guesses);
                var guess = GetGuess();
                ...
                else if(guess < number) // För stor
                {
                    ...
                    if((number - guess) < 10) Console.WriteLine("Det var nära");
                    if((number - guess) < 5) Console.WriteLine("Det var riktigt nära");
                    if((number - guess) < 3) Console.WriteLine("Du nuddade nästan");
                    if(guess > helperLow)
                    {
                        helperLow = guess;
                    }
                ...
                else // För litet
                {
                    ...
                    if((guess - number) < 10) Console.WriteLine("Det var nära");
                    if((guess - number) < 5) Console.WriteLine("Det var riktigt nära");
                    if((guess - number) < 3) Console.WriteLine("Det nuddade nästan");
                    if(guess < helperHigh)
                        helperHigh = guess;
                }
            }
